        ...
        <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
            ...
            <tbody>
                @foreach ($subjects as $subject)
                    <tr>
                        <td>{{ $loop->index + 1 }}</td>
                        <td>{{ $subject['name_' . $lang] }}</td>
                        <td>{{ $subject->grades['name_' . $lang] }}</td>
                        <td>{{ $subject->classrooms['name_' . $lang] }}</td>
                        <td>{{ $subject->degree }}</td>
                        <td>
                            <a href="{{ route('subjects.edit', $subject->id) }}" class="btn btn-info">
                                <i class="fa fa-edit"></i>
                            </a>
                            <button style="color: white" data-toggle="modal" data-target="#exampleModal{{ $subject->id }}"
