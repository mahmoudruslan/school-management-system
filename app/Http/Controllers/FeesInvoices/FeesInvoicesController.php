...
class FeesInvoicesController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('pages.fee_invoices.feeInvoices',compact('feeInvoices'));
    }
    ...
    public function store(Request $request,FeesRepository $f)
    {
        DB::beginTransaction();
        try{
            ...
            foreach($list_feesInvoices as $list_feesInvoice)
                {
                    $fee_invoice_id = $this->feesInvoices->create([
                        ...
                        'description' => $list_feesInvoice['description']
                        ]);
                    $fee = $f->getById($list_feesInvoice['fee_id']);
    ...
    public function update(Request $request, $id,FeesRepository $f)
    {
        try{
            ...
            $x = StudentAccount::where('fee_invoice_id',$id)->first();
